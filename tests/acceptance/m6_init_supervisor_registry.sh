...
if [ "$exit_code" -ne 0 ]; then
    printf '%s\n' "FAIL M6 QEMU launcher exited with $exit_code" >&2
    exit "$exit_code"
fi

serial_log="$repository_root/out/logs/m1-qemu-boot.log"
if [ ! -f "$serial_log" ]; then
    printf '%s\n' "FAIL M6 serial log was not created: $serial_log" >&2
    exit 4
fi
...
check_marker() {
    ...
    ')
    if [ -z "$line" ]; then
        printf 'FAIL M6 serial log does not contain the ordered marker: %s\n' "$marker" >&2
        exit 4
    fi
    last_line=$line
}
...
check_marker 'Nagi M6 supervisor START'
check_marker 'Nagi M6 manifest dependency order PASS'
check_marker 'Nagi M6 service health PASS'
check_marker 'Nagi M6 service registry START'
check_marker 'Nagi M6 echo@1 call PASS'
check_marker 'Nagi M5 syscall PASS'
check_marker 'Nagi M5 acceptance PASS'
check_marker 'Nagi M6 acceptance PASS'

printf '%s\n' 'PASS M6 acceptance: supervisor dependency order and real echo@1 registry call passed in the guest'
