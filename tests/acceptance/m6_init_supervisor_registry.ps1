...
foreach ($marker in $requiredMarkers) {
    ...
    if ($foundLine -lt 0) {
        throw "M6 serial log does not contain the ordered marker: $marker"
    }
    $lastLine = $foundLine
}
Write-Output 'PASS M6 acceptance: supervisor dependency order and real echo@1 registry call passed in the guest'
