...
if [ "$clean_exit" -ne 0 ]; then
    printf 'FAIL M10 clean exited with %s\n' "$clean_exit" >&2
    exit "$clean_exit"
fi

set +e
run_output=$(cd "$repository_root" && ./nagi desktop 2>&1)
run_exit=$?
set -e
printf '%s\n' "$run_output"
if [ "$run_exit" -ne 0 ]; then
    printf 'FAIL M10 desktop launcher exited with %s\n' "$run_exit" >&2
    exit "$run_exit"
fi
...
for marker in \
    ...
    'Nagi M10 acceptance PASS'; do
    line=$(printf '%s\n' "$serial" | awk -v marker="$marker" -v start="$last_line" '
        NR > start && index($0, marker) > 0 { print NR; exit }
    ')
    if [ -z "$line" ]; then
        printf 'FAIL M10 serial log does not contain ordered marker: %s\n' "$marker" >&2
        exit 4
    fi
    last_line=$line
done
