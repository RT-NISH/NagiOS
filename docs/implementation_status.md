...
**Last known repair checkpoint:** public CI run `35591875406` at `697dd08`
passed bootstrap, Mesa, package, and kernel, then stopped during target
compilation with `error[E0412]: cannot find type AtomicU32` in the new Nagi
condition implementation. The prior run exposed the target ABI gaps
`pthread_cond_timedwait`, `gai_strerror`, and `ioctl`; the current source
repair is complete but CI has not yet verified it because the atomic import
was missing. The corrective import is now added. No host libc, host rendering,
...
acceptance paths also remained PASS. M17 Servo Bootstrap is the active
 milestone. Public CI run `35591875406` is the current repair checkpoint:
bootstrap, Mesa, package, and kernel passed, then target compilation stopped
before link because `AtomicU32` was not imported by the new condition ABI. The
corrective import is now added; the Nagi condition, resolver, and ioctl repairs
remain recorded in ADR 0019. The required next evidence remains target link,
