...
**Last known repair checkpoint:** public CI run `35673531417` at `51deb09`
passed bootstrap, Mesa, package, kernel, and target compilation, then reached
final target linking. The `setjmp`, `longjmp`, and `readdir_r` repair is now
committed at `51deb09`, but this run failed earlier at the pinned Mesa
Softpipe archive step with exit code 2 and no compiler annotation. The same
Mesa source pin and build path passed in run `35671405076`, so the failure is
recorded as a reproducibility check still required, not as target-link evidence.
...
| M17 | Servo Bootstrap | BLOCKED | The pinned Servo/Surfman/libc/mio/socket2/tokio patch boundary, Nagi static Mesa/Softpipe build path, relibc-header preparation, and real Servo-to-Nagi Surface adapter are implemented. Public CI run `35673531417` at `51deb09` failed before target linking in the pinned Mesa Softpipe archive step with exit code 2 and no diagnostic annotation; the same path passed in `35671405076`. The run is not M17 acceptance evidence. M17 remains BLOCKED until the target link, UEFI loader, real QEMU, and real Servo-generated first-web-pixel evidence pass. See ADR 0019. |
