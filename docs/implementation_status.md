...
**Last known repair checkpoint:** public CI run `35593932419` at `085d912`
passed bootstrap, Mesa, package, kernel, and target compilation, then reached
the final target link. The exact next undefined symbols were `accept`,
`getsockopt`, and `lstat`; the current source repair connects them to the
Nagi POSIX facade and target-owned relibc backend. `accept` fails closed with
`ENOSYS` because the current Nagi network slice is client-only; `getsockopt`
reads real descriptor state; and `lstat` uses guest VFS metadata. No host
...
acceptance paths also remained PASS. M17 Servo Bootstrap is the active
 milestone. Public CI run `35593932419` is the current repair checkpoint:
bootstrap, Mesa, package, kernel, and target compilation passed, then final
linking exposed `accept`, `getsockopt`, and `lstat`. The target-owned Nagi
POSIX/relibc repair is implemented and pushed; the required next evidence
