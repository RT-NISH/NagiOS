...
**Last known repair checkpoint:** public CI run `35801744073` (#95) at
`d9db493` passed the target bootstrap, dependency boundary, Mesa Softpipe,
package, and kernel stages, then failed at final target linking with undefined
`_mesa_glthread_finish`, `printf`, and `getegid`. The current repair extracts
the real pinned Mesa glthread object into a dedicated target archive and adds
target-owned stdout and group-identity POSIX boundaries. This run is not
...
| M17 | Servo Bootstrap | BLOCKED | The pinned Servo/Surfman/libc/mio/socket2/tokio patch boundary, Nagi static Mesa/Softpipe build path, relibc-header preparation, and real Servo-to-Nagi Surface adapter are implemented. Public CI run `35801744073` (#95) at `d9db493` passed bootstrap, dependency boundary, Mesa, package, and kernel stages, then failed at final target linking on `_mesa_glthread_finish`, `printf`, and `getegid`; the current repair extracts the real pinned Mesa glthread object into a dedicated target archive and adds target-owned stdout and group-identity POSIX boundaries. The run is not M17 acceptance evidence. M17 remains BLOCKED until the target link, UEFI loader, real QEMU, and real Servo-generated first-web-pixel evidence pass. See ADR 0019. |
