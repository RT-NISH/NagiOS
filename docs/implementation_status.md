...
**Last known repair checkpoint:** public CI run `35668983049` at `e5925d9`
passed bootstrap, Mesa, package, kernel, and target compilation, then reached
final target linking. The exact undefined symbols were `rmdir`, `mkdir`, and
`opendir`; the current source repair adds real root-VFS mkdir/rmdir and a
root-directory snapshot adapter for the target POSIX boundary. No host libc,
...
| M17 | Servo Bootstrap | BLOCKED | The pinned Servo/Surfman/libc/mio/socket2/tokio patch boundary, Nagi static Mesa/Softpipe build path, relibc-header preparation, and real Servo-to-Nagi Surface adapter are implemented. Public CI run `35668983049` at `e5925d9` passed bootstrap, Mesa, package, kernel, and target compilation, then reached final target linking; the exact remaining diagnostics were `rmdir`, `mkdir`, and `opendir`. The current repair adds real root-VFS directory operations and target directory enumeration. M17 remains BLOCKED until target link, UEFI loader, real QEMU, and real Servo-generated first-web-pixel evidence pass. See ADR 0019. |
