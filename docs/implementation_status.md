...
**Last known repair checkpoint:** public CI run `35671405076` at `c1395a0`
passed bootstrap, Mesa, package, kernel, and target compilation, then reached
final target linking. The exact undefined symbols were `setjmp`, `longjmp`, and
`readdir_r`; the current source repair adds the x86-64 Nagi target context-save
ABI and a bounded readdir_r copy boundary over the real root-VFS directory
snapshot. No host libc, host filesystem, host rendering, or synthetic output
...
| M17 | Servo Bootstrap | BLOCKED | The pinned Servo/Surfman/libc/mio/socket2/tokio patch boundary, Nagi static Mesa/Softpipe build path, relibc-header preparation, and real Servo-to-Nagi Surface adapter are implemented. Public CI run `35671405076` at `c1395a0` passed bootstrap, Mesa, package, kernel, and target compilation, then reached final target linking; the exact remaining diagnostics were `setjmp`, `longjmp`, and `readdir_r`. The current repair adds the x86-64 Nagi context-save ABI and target directory enumeration reentrant copy boundary. M17 remains BLOCKED until target link, UEFI loader, real QEMU, and real Servo-generated first-web-pixel evidence pass. See ADR 0019. |
