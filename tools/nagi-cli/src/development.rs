...
fn load_state(
    ...
    let path = resolve_state_path(root, &workstream.state_file)?;
    let bytes = fs::read(&path).map_err(|error| {
        CliError::new(
            format!("cannot read {}: {error}", path.display()),
            EXIT_CONFIG_ERROR,
...
fn resolve_state_path(root: &Path, state_file: &str) -> Result<PathBuf, CliError> {
    ...
    let requested = root.join(state_file);
    let resolved = fs::canonicalize(&requested).map_err(|error| {
        config_error(format!(
            "cannot resolve state file {}: {error}",
            requested.display()
        ))
    })?;
    if !resolved.starts_with(&repository) {
        return Err(config_error(format!(
            "state file `{state_file}` resolves outside repository root"
        )));
    }
    Ok(resolved)
}
    ...
    if let Some(output) = output {
        ...
        if let Err(error) = file.write_all(&bytes) {
            ...
            let cleanup_detail = cleanup_error
                .map(|cleanup| format!("; cannot remove partial report: {cleanup}"))
                .unwrap_or_default();
            return Err(config_error(format!(
                "cannot write {}: {error}{cleanup_detail}",
                path.display()
            )));
        }
        Ok(vec![
            ...
            format!(
                "Logs: {}/{} readable; undefined symbols: {}; errors: {}; artifacts: {}",
                report["logs"].as_array().map_or(0, |items| items
                    .iter()
                    .filter(|item| item["exists"].as_bool() == Some(true))
                    .count()),
                logs.len(),
...
fn build_diagnostic_report(
        ...
        .any(|entry| entry["exists"].as_bool() == Some(true))
    {
        return Err(config_error(
            "diagnostic report has no readable logs; refusing to record empty evidence".into(),
        ));
    }
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn state_loading_rejects_symlinks_that_escape_the_repository() {
        use std::os::unix::fs::symlink;

        let root =
            std::env::temp_dir().join(format!("nagi-state-symlink-test-{}", std::process::id()));
        let outside = std::env::temp_dir().join(format!(
            "nagi-state-symlink-outside-{}.json",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        let _ = std::fs::remove_file(&outside);
        std::fs::create_dir_all(root.join(".dev/workstreams/alpha"))
            .expect("create state directory");
        std::fs::write(&outside, b"{}").expect("write external state target");
        symlink(&outside, root.join(".dev/workstreams/alpha/state.json"))
        ...
        let registry = json!({
            ...
            "workstreams": [{
                "id": "alpha",
                "owner": "test owner",
                "owner_branch": "codex/alpha",
                "recommended_worktree": "../NagiOS-alpha",
                "state_file": ".dev/workstreams/alpha/state.json",
                "dependencies": [],
                "allowed_paths": ["docs/**"],
                "forbidden_paths": [],
                "activation_gate": "test gate",
        ...
        let workstreams = load_registry(&root).expect("load registry");
        let result = load_state(&root, &workstreams[0], &workstreams);
        let error = result.expect_err("a state symlink cannot escape the repository root");
        std::fs::remove_dir_all(&root).expect("remove test repository");
        std::fs::remove_file(&outside).expect("remove external state target");
        assert!(
    ...
    #[test]
    fn diagnostics_fail_when_no_requested_log_can_be_read() {
        let root = std::env::temp_dir().join(format!(
            "nagi-diagnostic-missing-log-test-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("create diagnostic test directory");
        ...
        let result = diagnose(&args, &root);
        let report_exists = report.exists();
        let _ = std::fs::remove_dir_all(&root);
        let error = result.expect_err("diagnostics need at least one readable log");
        assert!(!report_exists, "failed diagnostics must not write a report");
        assert!(error.to_string().contains("no readable logs"), "{error}");
    }
